...
class relaVideoDataset(Dataset_base):
    def __init__(self, opt, train=True):
        super(relaVideoDataset, self).__init__(opt, train=train)

        self.nc = 3 if self.opt.dataset == "rela" else 4     
        if self.train:
            self.image_path = os.path.join('dataset/rela/', 'train')
        else:
            self.image_path = os.path.join('dataset/rela/', 'test')
...
class liutiVideoDataset(Dataset_base):
    def __init__(self, opt, train=True):
        super(liutiVideoDataset, self).__init__(opt, train=train)

        self.nc = 3 if self.opt.dataset == "liuti" else 4
        # print(self.nc)
        if self.train:
            self.image_path = os.path.join('dataset/liuti/', 'train')
        else:
            self.image_path = os.path.join('dataset/liuti/', 'test')
...
class VideoDataset(Dataset_base):
    ...
    def __init__(self, opt, train=True):
        ...
        if opt.dataset == 'mgif':
            ...
            vtrans = [vtransforms.Scale(size=128)]
        elif opt.dataset == 'flame':
            data_root = './dataset/flame_npy/'
            vtrans = [vtransforms.CenterCrop(size=120), vtransforms.Scale(size=128)]
        elif opt.dataset == 'bball':
            data_root = './dataset/bball/'
            vtrans = [vtransforms.CenterCrop(size=32),vtransforms.Scale(size=128)]
        elif opt.dataset == 'rotmni':
            data_root = './dataset/rotmni/'
            vtrans = [vtransforms.CenterCrop(size=28),vtransforms.Scale(size=128)]
        elif opt.dataset == 'mvmni':
            data_root = './dataset/mvmni/'
            vtrans = [vtransforms.CenterCrop(size=64),vtransforms.Scale(size=128)]
        if self.train:
            vtrans += [vtransforms.RandomHorizontalFlip()]
            vtrans += [vtransforms.RandomRotation()]
        ...
        threshold = self.window_size if opt.irregular else self.sample_size
        if opt.dataset in ['kth', 'sintel', 'ucf101', 'penn', 'flame','bball','rotmni','mvmni']:
...
def parse_datasets(opt, device):
    ...
    elif opt.dataset in ['liuti']:
        train_dataloader = DataLoader(liutiVideoDataset(opt, train=True),
                                      batch_size=opt.batch_size,
                                      shuffle=True,
                                      collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="train"))
        test_dataloader = DataLoader(liutiVideoDataset(opt, train=False),
                                     batch_size=opt.batch_size,
                                     shuffle=False,
                                     collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="test"))

    elif opt.dataset in ['rela']:
        train_dataloader = DataLoader(relaVideoDataset(opt, train=True),
                                      batch_size=opt.batch_size,
                                      shuffle=True,
                                      collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="train"))
        test_dataloader = DataLoader(relaVideoDataset(opt, train=False),
                                     batch_size=opt.batch_size,
                                     shuffle=False,
                                     collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="test"))

    elif opt.dataset in ['mgif', 'kth', 'penn', 'flame','bball','rotmni','mvmni']:
