...
def get_opt():
    ...
    parser.add_argument('--phase', default="test_met", choices=["train", "test_met"])
    ...
    # Need to be tested...
    parser.add_argument('--extrap', action='store_true', default=True,
    ...
    parser.add_argument('--dataset', type=str, default='rela', choices=["bball","rela","liuti","mgif", "hurricane", "kth", "penn","flame"])
...
def train(opt, netG, loader_objs, device):
    ...
    for epoch in range(opt.epoch):
        ...
        for it in range(n_train_batches):
            ...
            with torch.set_grad_enabled(True):
                loss_netD = opt.lamb_adv * netD_seq.netD_adv_loss(real.clone(), fake.clone(), input_real.clone())
                loss_netD += opt.lamb_adv * netD_img.netD_adv_loss(real.clone(), fake.clone(), None)
                loss_netD.backward()
            ...
            if (total_step + 1) % opt.log_print_freq == 0 or total_step == 0:
                ...
                log = f"Elapsed [{et}] Epoch [{epoch:03d}/{opt.epoch:03d}]\t" \
                      ...
                      f"Mse [{loss_netG.item():.4f}]\t" \
                      f"Adv_G [{loss_adv_netG.item():.4f}]\t" \
                      f"Adv_D [{loss_netD.item():.4f}]"
                print(log)

            if (total_step + 1) % opt.ckpt_save_freq == 0 or \
                    (epoch + 1 == opt.epoch and it + 1 == n_train_batches) or \
                    total_step == 0:
            ...
            if (total_step + 1) % opt.image_print_freq == 0 or total_step == 0:
                with torch.no_grad():
                    ...
                    if opt.extrap:
                        visualize.save_extrap_images(opt=opt, gt=gt, pred=pred,
                                                     path=opt.train_image_path, total_step=total_step)
                    else:
                        visualize.save_interp_images(opt=opt, gt=gt, pred=pred,
                                                     path=opt.train_image_path, total_step=total_step)
