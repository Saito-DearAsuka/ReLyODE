...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_tensor, h_cur, mask=None):
        ...
        reset_gate = torch.sigmoid(gamma)  # shape won't change
        update_gate = torch.sigmoid(beta)  # shape won't change
        ...
        '''torch.Size([8, 128, 13, 25])'''
        return h_next




class PositionEncoderVAE(nn.Module):
    def __init__(self, input_size, input_dim, output_dim, kernel_size=3):
        super(PositionEncoderVAE, self).__init__()
        self.height, self.width = input_size  
        ...
        #a convolution layer
        self.mean = nn.Conv2d(self.input_dim, self.output_dim, kernel_size=self.kernel_size, padding=1)
        self.var = nn.Conv2d(self.input_dim, self.output_dim, kernel_size=self.kernel_size, padding=1)
        self.relu = nn.ReLU()

    def forward(self, input_tensor):
        # input_tensor.shape = [b, t, c, h, w]
        # first frame：input_tensor[:, 0, :, :, :]
        first_frame = input_tensor[:, 0, :, :, :]

        mean = self.relu(self.mean(first_frame))
        std = self.relu(self.var(first_frame))
        std = torch.nn.functional.softplus(std) + 1e-8
        return mean, std


class VelocityEncoderVAE(nn.Module):
    def __init__(self, input_size, input_dim, output_dim, kernel_size=3):
        super(VelocityEncoderVAE, self).__init__()
        self.height, self.width = input_size  
        self.input_dim = input_dim  
        self.output_dim = output_dim  
        self.kernel_size = kernel_size
        ...
        self.conv = nn.Conv2d(self.input_dim, self.output_dim, kernel_size=self.kernel_size, padding=1)
        self.relu = nn.ReLU()
    ...
    def forward(self, input_tensor):
        ...
        #get all frames to active
        output_list = []
        for i in range(t):
            frame = input_tensor[:, i, :, :, :]
            output_frame = self.relu(self.conv(frame))
            output_list.append(output_frame)

        output = torch.stack(output_list, dim=1)

        #compute the dynamic info
        velocity = output[:, 1:, :, :, :] - output[:, -1:, :, :, :]
        velocity = torch.mean(velocity, dim=1)

        # first c/2 as mean, last c/2 as std
        c_v = velocity.size(1)
        mean = velocity[:, :c_v // 2, :, :]
        std = velocity[:, c_v // 2:, :, :]
        std = torch.nn.functional.softplus(std) + 1e-8
...
class Encoder_z0_ODE_ConvGRU_s_v_VAE(nn.Module):

    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, num_layers, dtype, batch_first=False,
                 bias=True, return_all_layers=False, z0_diffeq_solver=None, run_backwards=None):

        super(Encoder_z0_ODE_ConvGRU_s_v_VAE, self).__init__()
    ...
    def forward(self, input_tensor, time_steps, mask=None, tracker=None):
        ...
        last_yi, latent_ys, p_mean, p_std, v_mean, v_std = self.run_ode_conv_gru(
            input_tensor=input_tensor,
            mask=mask,
            time_steps=time_steps,
            run_backwards=self.run_backwards,
        ...
        # std_z0 = std_z0.abs()
        std_z0 = torch.nn.functional.softplus(std_z0) + 1e-8
        return mean_z0, std_z0, p_mean, p_std, v_mean, v_std
    ...
    def run_ode_conv_gru(self, input_tensor, mask, time_steps, run_backwards=True, tracker=None):
        ...
        position_encoder = PositionEncoderVAE(input_size=(h, w), input_dim=c, output_dim=c // 2).to(device)
        velocity_encoder = VelocityEncoderVAE(input_size=(h, w), input_dim=c, output_dim=c).to(device)

        p_mean, p_std = position_encoder(input_tensor)
        v_mean, v_std = velocity_encoder(input_tensor)

        epsilon_p = torch.randn_like(p_mean)
        epsilon_v = torch.randn_like(v_mean)

        position_features = p_mean + epsilon_p * p_std
        velocity_features = v_mean + epsilon_v * v_std

        # Set initial inputs
        prev_input_tensor = torch.cat([position_features, velocity_features], dim=1)
        # Time configuration
        # Run ODE backwards and combine the y(t) estimates using gating
        prev_t, t_i = time_steps[-1] + 0.01, time_steps[-1]
        latent_ys = []
        ...
        latent_ys = torch.stack(latent_ys, 1)
        return yi, latent_ys, p_mean, p_std, v_mean, v_std
