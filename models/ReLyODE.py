...
class ReLyODE(nn.Module):

    def __init__(self, opt, device):
        super(ReLyODE, self).__init__()
    ...
    def build_model(self):
        ...
        self.encoder_z0 = Encoder_z0_ODE_ConvGRU_s_v_VAE(input_size=input_size,
                                                         input_dim=base_dim,
                                                         hidden_dim=base_dim,
                                                         kernel_size=(3, 3),
                                                         num_layers=1,
        ...
        ##### Conv Decoder
        self.decoder = Decoder(input_dim=base_dim * 2, output_dim=self.opt.input_dim + 3, n_ups=self.opt.n_downs).to(
            self.device)
    ...
    def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, mask=None, out_mask=None):
        ...
        # print(w, resize)
        skip_conn_embed = self.encoder(skip_image).reshape(b, -1, h // resize, w // resize)

        ##### Conv encoding
        e_truth = self.encoder(truth.view(b * t, c, h, w)).reshape(b, t, -1, h // resize, w // resize)

        ##### ODE encoding
        first_point_mu, first_point_std, p_mean, p_std, v_mean, v_std = self.encoder_z0(input_tensor=e_truth,
                                                                                        time_steps=truth_time_steps,
                                                                                        mask=mask,
                                                                                        tracker=self.tracker)
        self.first_point_mu = first_point_mu
        self.first_point_std = first_point_std
        ...
        epsilon = torch.randn_like(first_point_mu)
        repara = first_point_mu + epsilon * first_point_std


        # first_point_enc = first_point_mu.unsqueeze(0).repeat(1, 1, 1, 1, 1)
        first_point_enc = repara.unsqueeze(0).repeat(1, 1, 1, 1, 1)
        # ==================================================================================== #
        ...
        pred_flows, pred_intermediates, pred_masks = \
            pred_outputs[:, :, :2, ...], pred_outputs[:, :, 2:2 + self.opt.input_dim, ...], torch.sigmoid(
                pred_outputs[:, :, 2 + self.opt.input_dim:, ...])
    ...
    def get_flowmaps(self, sol_out, first_prev_embed, mask):
        ...
        for t in time_iter:
            ...
            pred_flows = pred_flows + [pred_flow]
    ...
    def get_warped_images(self, pred_flows, start_image, grid):
        ...
        for t in range(warped_time_steps):
            ...
            pred_x = pred_x + [warped_x.unsqueeze(1)]  # b, 1, 3, h, w
    ...
    def compute_all_losses(self, batch_dict):
        ...
        var_v0 = self.v_std.pow(2)
        log_var_v0 = torch.log(var_v0 + 1e-8)
        kl_loss_v = -0.5 * torch.sum(1 + log_var_v0 - self.v_mean.pow(2) - var_v0)
        b_v, c_v, h_v, w_v = self.v_mean.size()
        kl_loss_v = kl_loss_v / (b_v * c_v * h_v * w_v)  

        var_p0 = self.p_std.pow(2)
        log_var_p0 = torch.log(var_p0 + 1e-8)
        kl_loss_p = -0.5 * torch.sum(1 + log_var_p0 - self.p_mean.pow(2) - var_p0)
        b_p, c_p, h_p, w_p = self.v_mean.size()
        kl_loss_p = kl_loss_p / (b_p * c_p * h_p * w_p)  
        var_z0 = self.first_point_std.pow(2)
        log_var_z0 = torch.log(var_z0 + 1e-8)
        kl_loss_z = -0.5 * torch.sum(1 + log_var_z0 - self.first_point_mu.pow(2) - var_z0)
        b_z, c_z, h_z, w_z = self.first_point_mu.size()
        kl_loss_z = kl_loss_z / (b_z * c_z * h_z * w_z) 

        loss = loss + 0.001 * kl_loss_v + 0.001 * kl_loss_p + 0.001 * kl_loss_z
