...
def Evaluation(opt, pred_list, gt_list):
    ...
    with torch.no_grad():
        ...
        for i, (pred_img, gt_img) in enumerate(zip(pred_list, gt_list)):
            ...
            # Calculate LPIPS
            gt_img_LPIPS = T2(gt_img).unsqueeze(0).to(device)
            pred_img_LPIPS = T2(pred_img).unsqueeze(0).to(device)
            avg_distance += model.forward(gt_img_LPIPS, pred_img_LPIPS)

            # Calculate MSE
            gt_img_MSE = T1(gt_img).unsqueeze(0).to(device)
            pred_img_MSE = T1(pred_img).unsqueeze(0).to(device)
